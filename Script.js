...
function add(event){
    const key = event.key.toUpperCase(); // Captures the key pressed
    console.log(key);
    if (regex.test(key)){
        if (trial.length < 5) {
            trial += key.toUpperCase();
            id = l * 5 + trial.length;
            id = id.toString();
            console.log(id);
            document.getElementById(id).innerHTML = key;
        }
    }else if (key === 'ENTER' && trial.length === 5) {
        if ((words.includes(trial))) {
            test(trial,l);
            l++;
            result(trial,l);
        }else{
    ...
    }else if (key === 'BACKSPACE' && trial.length > 0) {
        document.getElementById(id).innerHTML = '';
        ...
        for (let j = 0; j <= word.length; j++) {
           ...
           if (w[i] === word[j]) { 
               if (i === j && id <= (l*5 + 5)) { 
                ...
                document.getElementById(id).style.backgroundColor = '#957a03';
               }
           }
        }
